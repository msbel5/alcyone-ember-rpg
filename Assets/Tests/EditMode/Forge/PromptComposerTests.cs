...
namespace EmberCrpg.Tests.EditMode.Forge
{
    public sealed class PromptComposerTests
    {
        ...
        public void NpcPortrait_StyleGenre_AltersPromptDeterministically()
        {
            ...
            var high = PromptComposers.NpcPortrait(npc, Profile(WorldStyle.HighFantasyTolkien, WorldGenre.Pilgrimage));

            Assert.That(dark.Prompt, Does.Contain("DarkFantasyGrim"));
            Assert.That(high.Prompt, Does.Contain("HighFantasyTolkien"));
        ...
        {
            var profile = Profile(WorldStyle.LowFantasyMorrowind, WorldGenre.MerchantEmpire);
