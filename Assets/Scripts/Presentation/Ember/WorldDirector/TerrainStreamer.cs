...
{
    /// <summary>
    /// Daggerfall-style streaming world (Phase C): keeps a bubble of live terrain tiles around the player and
    /// loads/unloads them as the player walks, so the ground "renders as you go" and the world has no hard
    /// edge. Each tile is a seamless <see cref="RuntimeTerrainBuilder"/> chunk, deterministic from its tile
    /// coordinates + the world seed. The settlement sits on the flat central tiles; tiles further out are open
    /// hills. Manager/Observer pattern: one MonoBehaviour owns the tile dictionary and reacts to the player
    ...
    public sealed class TerrainStreamer : MonoBehaviour
    {
        ...
        private int _viewRadius = 1; // (2r+1)^2 live tiles around the player
        private uint _seed;
        ...
        public void Initialize(uint seed, BiomeKind biome, float tileSize = 256f, int viewRadius = 1)
        {
            _seed = seed;
            _biome = biome;
            ...
            Refresh(_current);
            Debug.Log($"[WorldDirector] terrain streaming online (tile {_tileSize:0}m, {(_viewRadius * 2) + 1}x{(_viewRadius * 2) + 1} bubble)");
        }
            ...
            for (int dx = -_viewRadius; dx <= _viewRadius; dx++)
            {
                for (int dz = -_viewRadius; dz <= _viewRadius; dz++)
                {
                    ...
                    if (!_tiles.ContainsKey(key))
                        _tiles[key] = RuntimeTerrainBuilder.BuildTile(transform, key.x, key.y, _tileSize, _biome, _seed);
                }
            }
