...
namespace EmberCrpg.Simulation.Process
{
    public sealed partial class JobAssignmentSystem
    {
        ...
        /// <summary>
        /// W33 (B06): tag-count IO tick — _activeOrders mixes DIFFERENT sites' orders, so the
        /// caller supplies a per-site resolver and each order writes its OWN site's container.
        /// The single-inventory overloads above stay byte-identical for their callers.
        /// </summary>
        public int TickAssignedJobs(
            ActorStore actors,
            JobBoard board,
            WorksiteStore worksites,
            Func<SiteId, IRecipeInventory> ioForSite,
            WorldEventLog eventLog,
            GameTime now)
        {
            if (actors == null)
            ...
            if (worksites == null)
                throw new ArgumentNullException(nameof(worksites));
            if (ioForSite == null)
                throw new ArgumentNullException(nameof(ioForSite));
            if (eventLog == null)
                throw new ArgumentNullException(nameof(eventLog));
            ...
            foreach (var pair in _activeOrders)
            {
                if (recipeSystem.Tick(pair.Value, ioForSite(pair.Value.SiteId), eventLog))
                    completed.Add(pair.Key);
            }
        ...
        // W33 (B06): same clone-proof preflight over the tag-count seam — the probe never
        // touches the live container (CloneForPreflight contract).
