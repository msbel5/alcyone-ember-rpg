...
| Faz 8 slice 2 | CRPG | Data-only new spell proof | Add a spell row with no C# branch and event-log proof | Magic services/catalog/tests | queued (SpellExecutionService still routes through legacy SpellEffectResolutionService; data-driven SpellResolver is wired via DomainSimulationAdapter.TryCastSpell command sink) |
