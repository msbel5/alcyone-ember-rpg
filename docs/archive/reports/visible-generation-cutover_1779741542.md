# Visible Generation Cutover — PR #214 Rescue Final Report

## PR
#214 OPEN draft=true docs/codex-mission-v2->main https://github.com/msbel5/alcyone-ember-rpg/pull/214

## Commits Before This Report
da7b71a0 docs(prd): AAA Uplift Sprint A — Codex mission prompt (Smithing + Tavern)
585533f3 docs(prd): plan AAA scene quality uplift (post-cutover, 5 sprints)
bcc733a5 chore(ui): collapse duplicate UI tokens + IUiPanel surface; clean uncommitted artifacts
6d2d5f83 report: add aaa playability rescue evidence
3122b0b0 fix: rescue playability visuals and character creation
d0cc9d73 fix: align visible generation cutover runtime
...
.gitignore                                         |   11 +
 ...
 .../CharacterCreationController.Rendering.cs       |  458 +
 .../CharacterCreationController.Rendering.cs.meta  |    2 +
 .../CharacterCreationController.cs                 |  602 ++
 ...
 .../Diagnostics/EmberProofScreenshotDriver.cs      |  145 +
 ...
 .../Ember/Worldgen/WorldgenViewController.cs       |  195 +
 ...
 405 files changed, 58483 insertions(+), 23189 deletions(-)

## Tests
- fallback harness: PASS, 1420 passed / 1423 total / 3 skipped, Reports/validation-fallback-rescue_route2.log
...
## Acceptance Snapshot
A. Compile/static: PASS. No compile errors; Domain/Simulation static guard found no real UnityEngine imports; UIElements refs are backend-only.
B. Boot/Loading: PASS in built-player proof; Loading + Worldgen visible at Reports/screens/rescue-pr214/worldgen_loading.png.
C. CharacterCreation: PASS in PlayMode + screenshot; full dossier intent persisted and no camera/stuck state at Reports/screens/rescue-pr214/character_creation.png.
D. Worldgen: PASS in PlayMode; auto-advance deterministic path requests SmithingOverworld; modal/log visible in screenshot.
E. Scene Playability: PASS for rescue scope. Smithing/Tavern rebuilt with non-white materials, readable billboards, and spawn camera inside room.
F. Git: PR #214 remains draft and unmerged; commit/push follows this report.
...
- Canonical batch: Reports/screens/aaa_*_1779715425.png
- Windows64 exe: Builds/Windows64/alcyone-ember-rpg.exe

## Fixes Landed
- Added route/playability tests for material assignment, spawn/camera comfort, readable art assets, worldgen auto-advance, and CharacterCreation dossier intent.
- Added deterministic scene material library entries for Smithing/Tavern floors and walls.
- Made terrain assets scene-keyed to stop shared floor/layer overwrites.
- Preserved authored non-white materials in the surface sanitizer.
- Added player spawn and hero camera anchors; corrected spawn placement to avoid wall starts while keeping actor comfort distance.
- Persisted CharacterCreation class, alignment, background, skills, attributes, birthsign, portrait JSON, and seed into EmberWorldGenIntent.
- Made Worldgen auto-advance deterministic/testable.
- Fixed fallback harness on Windows/WSL by enforcing LF shell scripts and detecting Windows dotnet.exe.

## Unity / MCP
- MCP status: Unity-specific MCP unavailable; headless Unity and built player used.
- Console error count: 0 compile/test failures after fixes. Benign Unity license/AI tracing warnings remain in logs.
- grep status: Packages/com.unity.ai.assistant/ThirdParty~/ripgrep/rg_win.exe verified present earlier in this rescue pass.

## Recommended Next Step
Keep PR #214 draft until reviewer verifies the visual direction in the built player. Next PR should replace placeholder block props with curated/generated room dressing and improve HUD contrast, not reopen the routing/validation foundation.