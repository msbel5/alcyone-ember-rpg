...
PHASE   : P1.5 — DESIGN ALIGNMENT  (rebuild HUD/dialog/clock to match docs/design-system + Reference/PRDs)
BRANCH  : feat/p15-design-alignment  (new — main fallback intact)
UPDATED : 2026-05-29
```

**HONEST RESET 2026-05-29.** Prior "P1 DONE" merge to main was premature. Confirmed against `docs/design-system/colors_and_type.css` + `Reference/PRDs/PRD_frontend_*_v1.md` (action_bar, ask_about, ask_dm, clock_widget, dialog_system, character_record):
- ✅ T1 themed floors REAL (8 distinct env_*.png md5s painted onto Unity Terrain via SceneEnvironmentDresser, NOT Tile_*.mat _BaseMap) — but visual regression reports: SeasonFarm green missing, stray gray cube props (not magenta — those are gone, T4 was fine).
- ✅ T2 generation REAL (splash/logo/UI/portrait/env real, fallbacks surfaced).
- ❌ **T3 HUD WAS THE WRONG FIX.** I removed CombatDungeon's bottom HEALTH/FATIGUE/MANA bars + standardized top-pills+9-slot. The PRD says EXACT OPPOSITE: bottom-left labeled bars (red/yellow/blue) + bottom-center BG1 12-button context action bar (PRD_frontend_action_bar_v1). Must rebuild.
- ✅ T4 goblin checker fixed (PIL flood-fill).
- ❌ **T5 LLM round-trip NOT verified.** Build integrates LLamaSharp 0.27 + Qwen GGUF + native llama.dll, but USE_LLAMASHARP define currently OFF in Windows64BuildMenu.cs (reverted to protect main) and no in-game ask-DM screenshot has captured a real Qwen response. Sync Complete() also caused black-screen UX → needs Task.Run thread-off.
- ❌ Ask About modal is a stub, NOT the deterministic-from-conversation_state PRD.
- ❌ Ask DM free-text panel + advisor_view: not built.
- ❌ Clock widget ("Day N, HH:MM" + right-click pause): missing.
- ❌ Consul Fate / Character Record / cross-scene NPC interactions: missing or broken.

**P1.5 backlog: 9 design-alignment tasks** (T-HUD ▶ NOW, T-Dialog-AskAbout, T-Clock, T-ConsulFate, T-AskDM, T-LLM-Verify, T-Props, T-NPC-Interact, T-Merge). Token-preservation: one task → smallest slice → batchmode build → screenshot proof → commit → advance pointer. See `validation-output/goal_condition_v2.txt` for the canonical session-start prompt.
...
| T3 | ~~HUD consistency (top pills + 9-hotbar)~~ — **WRONG DIRECTION**, superseded by T-HUD in P1.5 backlog | ❌ REVERSE | PRD_frontend_action_bar_v1: bottom-left HEALTH/FATIGUE/MANA labeled bars + bottom-center 12-button context action bar. Rebuild per PRD; delete the top-pills+9-slot frame I shipped. |
...
| T5 | **AI DM conversation round-trip** — talk to the embedded LLM in-game (dialog / oracle / ask-DM), get a coherent response, offline-capable — superseded by T-LLM-Verify in P1.5 backlog (sync Complete blocks main thread → black screen; needs Task.Run + screenshot proof of a real Qwen reply) | ❌ NOT VERIFIED | Pillar B is implemented: `NativeLlmClient` (on-device Qwen2.5-1.5B-Instruct GGUF, lazy-downloads on first use) + `LocalQwenClient` fallback; `AskDmService`/`NpcDialogueService`/`AskAboutService`/`ToolUseService` wired. T5 SCOPE CORRECTED: it is MORE than verify. `AskDmService` is a deterministic SHELL (returns a templated string, NOT LLM-wired). Must first confirm whether in-game dialog (TavernDialog ask-about) routes through `NativeLlmClient` (via `NpcDialogueService` / AiDm `ToolUseService`/`NarrationServices`) or the shell, and WIRE the real LLM path if not — THEN verify the round-trip (the `LocalQwenClient` fallback covers the no-model case, so it can be proven offline). BLOCKER FOUND: LLM fully wired (DomainSimulationAdapter->LlmRoutingService->NativeLlmClient, async dialog + thinking-placeholder; ForgeBootstrap registers it); native backend (llama.dll+ggml*.dll) + GGUF model (StreamingAssets/Models/qwen2.5-1.5b) PRESENT — but managed LLamaSharp.dll is MISSING from Assets/Plugins/x86_64 AND USE_LLAMASHARP is NOT defined, so inference is compiled out -> dialog falls back to canned text, not a real exchange. COMPLETE T5: (1) add LLamaSharp.dll 0.27.0 managed binding (NuGet) to Assets/Plugins/x86_64 [README documents it]; (2) add USE_LLAMASHARP to build scriptingDefines; (3) rebuild + driver to trigger a TavernDialog ask + capture the real response. DO NOT define USE_LLAMASHARP before the DLL exists (compile break = regressed build). DEP SCOPE (downloaded LLamaSharp 0.27 nupkg, netstandard2.0 in TEMP): LLamaSharp.dll needs 9 transitive managed deps (CommunityToolkit.HighPerformance 8.4.2, Microsoft.Bcl.AsyncInterfaces, Microsoft.Bcl.Memory, Microsoft.Extensions.AI.Abstractions, Microsoft.Extensions.Logging.Abstractions, System.Linq.Async, System.Linq.AsyncEnumerable, System.Numerics.Tensors, System.Text.Json) — AND conflicts with the build's EXCLUDE_BCL_MEMORY/EXCLUDE_BCL_NUMERICS defines (LLamaSharp needs Bcl.Memory + Numerics.Tensors). So T5 = a real dependency integration (fetch 9 deps+subdeps, resolve the BCL-exclude conflict, build-iterate for Unity-Mono compat), NOT a drop-in. Do as a focused step; ALWAYS revert USE_LLAMASHARP on any compile failure to protect the working build. |
...
**▶ NOW = T-Dialog-AskAbout slice 2** (deterministic-topic provider swap: replace `EmberWorldHost.GetTopics()` static `{rumors, work, trade, fate}` with `AskAboutService` / `SliceWorldState.Topics`-fed real topic IDs; the Showroom screen already shows real IDs `{embers, gate, watch}` via a different path, so wiring is partial — make TavernDialog use the same source. Then F4-to-open hotkey + auto-close on `IDialogSource.IsThinking==false && GetTopics().Count==0`). T-Dialog-AskAbout slice 1 ✅ shipped 2026-05-30 (`p15dlg02`): DialogBoxPanel now has void-cool 92% backing + gold hairline border + parchment text + panel-brown portrait frame; NPC line + topics readable across TavernDialog + Showroom screenshots; portrait slot reads as intentional placeholder. T-HUD slice 1 ✅ shipped 2026-05-30 (commit forthcoming, branch `feat/p15-design-alignment`, build `p15hud02`): bottom-left labeled HEALTH/FATIGUE/MANA bars + bottom-center 12-button BG1 STANDARD strip (ATK CAST TALK INV CHAR MAP JOURN SRCH STLTH MODAL FORM EQUIP) + top-left Tick/Day status; user-verified across 5/10 scene-tour `_ui` screenshots (Smithing, Combat, Tavern, Colony, Farm) — all show the new frame correctly bottom-anchored. Action-level state machine (UAW_QSPELLS / modal toggles / formation presets / group-select) is a later T-HUD slice that wires into `IPlayerCommandSink`. Reading order: §0 protocol → §1 reset above → PRD on starting a backlog task only. P1.5 supersedes the T3-T6 row labels above (kept as audit trail). T4 CHECKER FIXED** (commit fbdea443, genesis32-verified): `goblin_fixed.png` baked checker background removed → transparent; goblins render clean; **done-when criterion (3) zero magenta/checker now MET** (magenta already absent everywhere). Residual minor (NOT magenta/checker, not blocking #3): gray untextured cube props (Oracle/Showroom/TavernFlavour) + the intentional NPC-portrait placeholder — fold into a later polish pass. T1 (themed floors) + T2 (generation real) DONE & build-verified:
