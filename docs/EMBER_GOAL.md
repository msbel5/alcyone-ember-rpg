...
| T5 | **AI DM conversation round-trip** — talk to the embedded LLM in-game (dialog / oracle / ask-DM), get a coherent response, offline-capable | BUILT, verify | Pillar B is implemented: `NativeLlmClient` (on-device Qwen2.5-1.5B-Instruct GGUF, lazy-downloads on first use) + `LocalQwenClient` fallback; `AskDmService`/`NpcDialogueService`/`AskAboutService`/`ToolUseService` wired. T5 SCOPE CORRECTED: it is MORE than verify. `AskDmService` is a deterministic SHELL (returns a templated string, NOT LLM-wired). Must first confirm whether in-game dialog (TavernDialog ask-about) routes through `NativeLlmClient` (via `NpcDialogueService` / AiDm `ToolUseService`/`NarrationServices`) or the shell, and WIRE the real LLM path if not — THEN verify the round-trip (the `LocalQwenClient` fallback covers the no-model case, so it can be proven offline). BLOCKER FOUND: LLM fully wired (DomainSimulationAdapter->LlmRoutingService->NativeLlmClient, async dialog + thinking-placeholder; ForgeBootstrap registers it); native backend (llama.dll+ggml*.dll) + GGUF model (StreamingAssets/Models/qwen2.5-1.5b) PRESENT — but managed LLamaSharp.dll is MISSING from Assets/Plugins/x86_64 AND USE_LLAMASHARP is NOT defined, so inference is compiled out -> dialog falls back to canned text, not a real exchange. COMPLETE T5: (1) add LLamaSharp.dll 0.27.0 managed binding (NuGet) to Assets/Plugins/x86_64 [README documents it]; (2) add USE_LLAMASHARP to build scriptingDefines; (3) rebuild + driver to trigger a TavernDialog ask + capture the real response. DO NOT define USE_LLAMASHARP before the DLL exists (compile break = regressed build). DEP SCOPE (downloaded LLamaSharp 0.27 nupkg, netstandard2.0 in TEMP): LLamaSharp.dll needs 9 transitive managed deps (CommunityToolkit.HighPerformance 8.4.2, Microsoft.Bcl.AsyncInterfaces, Microsoft.Bcl.Memory, Microsoft.Extensions.AI.Abstractions, Microsoft.Extensions.Logging.Abstractions, System.Linq.Async, System.Linq.AsyncEnumerable, System.Numerics.Tensors, System.Text.Json) — AND conflicts with the build's EXCLUDE_BCL_MEMORY/EXCLUDE_BCL_NUMERICS defines (LLamaSharp needs Bcl.Memory + Numerics.Tensors). So T5 = a real dependency integration (fetch 9 deps+subdeps, resolve the BCL-exclude conflict, build-iterate for Unity-Mono compat), NOT a drop-in. Do as a focused step; ALWAYS revert USE_LLAMASHARP on any compile failure to protect the working build. ATTEMPTED 2026-05-29 on branch `feat/llamasharp-t5`: downloaded LLamaSharp 0.27 + 4 deps to `Assets/Plugins/NuGet/` (CommunityToolkit.HighPerformance 8.4.2, Microsoft.Extensions.AI.Abstractions 10.4.1, System.Linq.Async 7.0.0, System.Numerics.Tensors 10.0.5; netstandard2.0). Build fails with deep VERSION CONFLICT: new v10.x deps reference `Microsoft.Bcl.AsyncInterfaces` v10.0.0.0 exactly, but project has v10.0.0.1 (bundled in `Packages/com.unity.ai.assistant/Plugins/Shared/`) + v8.0.0.0 (existing `Assets/Plugins/NuGet/`). Unity/Mono lacks binding redirects, so the type-identity mismatch propagates: `NpcMemoryStore`/`EquipmentState`/`PriceLedger`/.../even Unity UGUI's `ToggleGroup` all fail with CS0012 "must add reference to `Microsoft.Bcl.AsyncInterfaces, Version=10.0.0.0`". NEXT ATTEMPTS (in order of risk): (a) download Microsoft.Bcl.AsyncInterfaces 10.0.0 (assembly v10.0.0.0) → overwrite NuGet/v8; (b) if Unity still prefers ai.assistant's v10.0.0.1, remove ai.assistant's bundled BCL DLLs from `Packages/com.unity.ai.assistant/Plugins/Shared/` (modifying a package — risky); (c) downgrade to OLDER LLamaSharp (~0.18–0.20) that targets v8 BCL matching the project environment. Branch state: 5 DLLs staged (LLamaSharp + 4 deps), USE_LLAMASHARP still undefined (build fails before reaching that step). **MAIN IS SAFE** (P1 foundation merged at 8046eba7). ATTEMPT 2 (per-user "match all versions" strategy): downloaded Bcl.AsyncInterfaces 10.0.0 + Bcl.Numerics 10.0.0 + Linq.AsyncEnumerable 10.0.0 + Interactive.Async 6.0.1 (all asm v10.0.0.0 except Interactive @ 6.0.0.0); overwrote NuGet's v8 Bcl.AsyncInterfaces; replaced ai.assistant's Bcl.AsyncInterfaces v10.0.0.1 → v10.0.0.0 (later reverted as noop — ai.assistant's BCLs are excluded on Unity 6.5+ via `!UNITY_6_5_OR_NEWER` defineConstraint). Nuked Library/Bee + ScriptAssemblies. **STILL FAILS** with the same CS0012 (must add v10.0.0.0 Bcl.AsyncInterfaces). DECISIVE NEW FINDING from `Library/Bee/.../EmberCrpg.Domain.rsp`: Unity adds `-r:Bcl.Memory`, `-r:Bcl.Numerics`, `-r:Bcl.TimeProvider` BUT **NOT** `-r:Bcl.AsyncInterfaces` — Unity 6 is **selectively excluding Bcl.AsyncInterfaces from auto-reference** even though the DLL is loaded and the `.meta` is identical to working DLLs. Strong evidence Unity 6 ships its own internal Bcl.AsyncInterfaces handling that overrides project copies, but its identity doesn't match what LLamaSharp's v10 deps demand. **PIVOT**: path (c) is now the right move — try **older LLamaSharp (~0.18–0.20)** that targets v8-era BCL, which matches Unity 6's bundled environment + the project's pre-T5 working state. |
