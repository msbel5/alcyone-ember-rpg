...
| T5 | **AI DM conversation round-trip** — talk to the embedded LLM in-game (dialog / oracle / ask-DM), get a coherent response, offline-capable | BUILT, verify | Pillar B is implemented: `NativeLlmClient` (on-device Qwen2.5-1.5B-Instruct GGUF, lazy-downloads on first use) + `LocalQwenClient` fallback; `AskDmService`/`NpcDialogueService`/`AskAboutService`/`ToolUseService` wired. T5 = interactive in-game round-trip verify (model present → ask-DM/dialog → coherent reply shown). |
