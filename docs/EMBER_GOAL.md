...
- T1.5 NEXT (THE GATE) — make env entries GENERATE the RIGHT way.
  ...
  - **Right fix:** generate env floors via a per-scene/worldgen loading pass OR the existing incremental top-up — investigate how the other ~31 non-boot manifest entries get generated (`VisibleGenerationFlow.RunCoreAssetTopUpAsync` is the "top-up" entry; find what currently drives it post-boot) and route the 8 `env_<scene>` entries through it. Generated assets cache (flow skips `Cached`), so it's a one-time cost.
