...
| 8 | AI/DM | `Assets/Scripts/Simulation/AiDm/NpcFlavourService.cs` | NPC ambient bark generation. Falls back to Faz 9 `DialogueTemplate` if LLM rejected or over budget. | `NpcFlavourServiceTests` | queued |
| 9 | AI/DM | `Assets/Scripts/Simulation/AiDm/DmNarrationService.cs` | DM narration of accepted tool-call outcomes. Falls back to templated narration. | `DmNarrationServiceTests` | queued |
| 10 | AI/DM | `Assets/Scripts/Domain/AiDm/ConsultFateOutcomeBucket.cs` | Stable string outcome bucket id (`favourable`, `neutral`, `setback`). Data-driven. | `ConsultFateOutcomeBucketTests` | queued |
| 11 | AI/DM | `Assets/Scripts/Simulation/AiDm/ConsultFateService.cs` | Player presses DM key → service selects bucket from deterministic seeded roll → DM narrates → tool calls validated → outcomes applied. | `ConsultFateServiceTests` | queued |
| 12 | AI/DM | `Assets/Scripts/Simulation/AiDm/StorytellerCheckpointSystem.cs` | RimWorld-lineage condition checkpoints: prosperity / disaster / opportunity beats triggered when sim metrics cross thresholds. | `StorytellerCheckpointSystemTests` | queued |
